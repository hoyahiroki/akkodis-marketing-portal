...
# -*- coding: utf-8 -*-
"""
ToBe ワイヤーフレーム v4 生成スクリプト（判読性最優先・大判レンダリング）
- v3（build_tobe_wf_v3.py）を土台に、社長3回目差戻し＋一次情報再検証
  （sharepoint-verify-v4.md V8〜V13）を反映した改訂版。**指示された変更のみ**。
- v3→v4の主な変更点（詳細は wf-v4-directives.md）:
  1) クイックリンクを4列に戻す（V8再検証：3列上限はセクション列レイアウト
     ＝強調表示されたコンテンツ/リスト等をセクションの列に並べる場合のみ）。
     対象：トップ4カテゴリカード／クライアントAKKODiS企業向けWebページ4リンク／
     キャンディデートSocial Media 4リンク。
  2) トップT-4を縦積みに変更（①AI-1自己チェック→②手順ガイド→③業務依頼フォーム
     を上から下へ全幅3ステップ。3カラム横並びをやめる）。
  3) ブランド「ブランドの使用について」のサブブロックを別セクション化
     （V9：背景色はセクション単位のみ設定可能なため、サブブロック単位の色分けは
     セクション分割で実現）。
  4) ブランド画像素材はテキストweb partのみに簡素化（クイックリンク併用をやめる。
     テキスト内に4見出し＋各リンクを箇条書き）。
  5) ブランド各種テンプレートは5つの「強調表示されたコンテンツ web part」に分割
     （V10：グループ表示機能が無いため、カテゴリごとに個別配置＋見出し＋フィルタ）。
  6) キャンディデートAKKODiS公式HP：「AKKODiS People」への業務依頼フォームリンクは
     クイックリンクの1タイル=1リンク制約のため副リンクを入れられない（V11）→
     テキストweb partに変更し複数ハイパーリンクで表現。Social Mediaはクイックリンク4列。
  7) パートナーはv3のまま変更なし。
"""
import os
...
OUT = "/workspace/akkodis-marketing-portal/deliverables/1784786903168-17720/wf_v4"
...
def titlebar(name, idx, total, sub=""):
    return f'''<div class="titlebar">
      <div class="name">ToBe ワイヤーフレーム v4　｜　{name}　（{idx}/{total}）</div>
      <div class="sub">{sub or "AKKODiSマーケティングポータル・承認用WF（社長3回目差戻し反映）"}</div>
    </div>'''
...
def changes_box(lines, head="このページの変更点（v4・社長3回目差戻し反映）", pin="v4変更点"):
...
def build_top():
    blocks = [
        ...
        footnote("ToBe WF v4 — 1. トップ（ホーム） — SharePoint UI再検証 v4（sharepoint-verify-v4.md V8〜V13）準拠"),
    ]
...
def build_brand():
    blocks = [
        ...
        toc([
            ...
            {"label": "各種テンプレート", "target": "強調表示されたコンテンツ web part × 5（カテゴリごとに個別配置）"},
        ...
        hc5_block(
            11, "各種テンプレート（v4：5つの強調表示されたコンテンツ web partに分割）",
            [
                ("PowerPointテンプレート", "フィルタ: コンテンツタイプ＝PowerPoint プレゼンテーション"),
                ("Wordテンプレート", "フィルタ: コンテンツタイプ＝Word 文書"),
                ("メール署名テンプレート", "フィルタ: マネージドプロパティ（カテゴリ列）＝メール署名"),
                ("Formsテンプレート", "フィルタ: マネージドプロパティ（カテゴリ列）＝Forms"),
                ("バーチャル背景素材", "フィルタ: マネージドプロパティ（カテゴリ列）＝バーチャル背景"),
            ],
            note="【v4】V10検証の結果、強調表示されたコンテンツ web partは<b>1web partにつき1つの動的クエリ</b>であり、"
            "結果をカテゴリ別に内部でグループ化・タブ分けして表示する機能（Group by相当）はドキュメント上に存在しないと判明。"
            "「1つにまとめる」設計を撤回し、<b>カテゴリごとに5つのweb partを個別配置</b>（各に見出し＋コンテンツタイプ/マネージドプロパティのフィルタ）する。"
            "セクション列は3列以内のため、WF上は3個＋2個の2段で自動的に折り返す。「ワンクリックで動的に多条件フィルタできる」という誤った表現は使わない（R2）。",
            badges=[("R2", "rule"), ("v4改訂", "v4"), ("V10", "rule")],
        ),
        (lambda: f'''<div class="sec">
          <div class="sechead">
            <div>
        ...
        changes_box([
            ...
            "【v4】「各種テンプレート」を<b>5つの「強調表示されたコンテンツ web part」に分割</b>した（V10検証：グループ表示機能が無いため、"
            "PowerPoint/Word/メール署名/Forms/バーチャル背景の各カテゴリごとに個別web部品＋見出し＋フィルタを配置。1つにまとめる設計は撤回）。",
        ...
        footnote("ToBe WF v4 — 2. ブランドライブラリ（1枚に統合）— SharePoint UI再検証 v4（V9・V10・V11）反映"),
    ]
...
def build_client():
    blocks = [
        ...
        (lambda: f'''<div class="sec">
          ...
          日本語版／英語版が対の資料は言語列で1エントリに集約。カテゴリに「事例資料」を維持。【v4】本セクションは変更なし：3列上限はセクションの列レイアウトの制約であり、
          本パーツは単一のリスト web part内のビュー絞り込みチップで表現しているため該当しない（該当リストは3列制限の対象外）。</div>
        </div>''')(),
        sec(
        ...
        changes_box([
            ...
            "（v3までの継続・変更なし）「公開資料ライブラリ」は単一のリスト web part（行＋カテゴリ/言語/更新日列＋ビュー絞り込みチップ）のまま。"
            "3列制限はリストの列表示ではなくビュー絞り込みのため元々非該当（変更不要）。旧SECTION5案内は枠内の1行注記に簡素化済み。「事例資料」カテゴリ維持。",
        ]),
        footnote("ToBe WF v4 — 3. クライアント向け — SharePoint UI再検証 v4（V8）反映"),
    ]
...
def build_candidate():
    blocks = [
        ...
        (lambda: f'''<div class="sec">
          <div class="sechead">
            <div>
              ...
              <div class="title">AKKODiS公式HP（v4：AKKODiS Peopleをテキスト化）</div>
            </div>
            <div class="badges"><span class="badge v4">v4改訂</span><span class="badge rule">V11</span></div>
          </div>
        ...
        footnote("ToBe WF v4 — 4. キャンディデート向け — SharePoint UI再検証 v4（V8・V11）反映"),
    ]
...
def build_partner():
    blocks = [
        ...
        changes_box([
            "【v4】本ページは<b>変更なし</b>（v4変更指示：パートナーはv3のまま）。",
            "（v3までの継続）3パートナーのサムネイルカード（役割A）を削除し、ページ内目次（クイックリンク web part）のみを残した。P-1（ASP Teamの画像枠是正）・P-2（ページ内目次新設）は維持。",
        ], head="このページの変更点（v4：変更なし・v3から継続）"),
        footnote("ToBe WF v4 — 5. グローバルブランドパートナーシップ（v3から変更なし）"),
    ]
...
# PDF結合（1ページ＝1WF）
# ---------------------------------------------------------------------------
def build_pdf(png_paths, out_name="ToBe_WF_v4_承認用.pdf"):
